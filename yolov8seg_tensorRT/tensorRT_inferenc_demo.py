...
def main():
    engine_file_path = './yolov8nseg_relu_80class_dfl.trt'
    ...
    with get_engine_from_bin(engine_file_path) as engine, engine.create_execution_context() as context:
        ...
        result_img = np.clip(np.array(orig_image) + np.array(mask) * 0.8, a_min=0, a_max=255)

        cv2.imwrite('./test_result_tensorRT.jpg', result_img)
